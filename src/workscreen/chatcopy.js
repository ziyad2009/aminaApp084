...
import {Box,Avatar,HStack,VStack,Spacer,Center,Text,Button, Radio,Modal,Stack, Input} from 'native-base'
...
const chikchatRoomMsg=async(roomID)=>{ 
  ...
  //   };
   ...
   // const seconds = remainingTime % 60
    ...
    return <Text>{minutes-elapsedTime}</Text>
  }
   
  const addextraTime=(hours)=>{
                     ...
                     {/* <Button bgColor={Colors.AminaButtonNew} size={'lg'} mb='1.5' w='full'
                        onPress={() =>   setShowModal(true) }> تمديد</Button> */}
                         <CustomButton
                          ...
                          onPress={() =>  addextraTime(babysetter.hours)  }
                            />
...
<Modal.Footer alignItems={'center'} justifyContent='center'>
  ...
  <Box alignItems={'flex-start'}  w={Metrics.WIDTH*0.9466} ml='5' mr='1' mt={5}   rounded='lg'>
    {/* <Text fontFamily={Platform.OS==='android'?Fonts.type.medium:Fonts.type.base} fontSize='lg' textAlign={'center'} mb={2}>اكمل الدفع عن طريق</Text> */}
     <Image source={Images.madacard} resizeMode='contain' style={{width:50,height:50}} />
     <Input placeholder='card number' textAlign={'center'} onChangeText={()=>console.log("test card ",)}
         fontSize={15}  borderColor={Colors.black} borderWidth={.2} w={Metrics.WIDTH*0.832} maxLength={14} />
     <HStack flexDirection={'row'} justifyContent='space-between' width={Metrics.WIDTH*0.8336} >
     <MaskInput
            value={cardExpiry}
            mask={DATEMASK}
            maxLength={6}
            ...
            // setDValue(unmasked); // you can use the masked value as well
              ...
              console.log("state=",cardExpiry); // "R$ 1.234,56"
              console.log("value=",unmasked); // "123456"
            }}
          />
     <Input placeholder='CVC' textAlign={'center'}  onChangeText={()=>console.log("test card ",)} borderColor={Colors.white} borderWidth={.2} w={Metrics.WIDTH*0.332} />
     </HStack>
